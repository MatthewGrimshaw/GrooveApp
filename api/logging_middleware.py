...
class DatabaseLoggingMiddleware:
    ...
    def __enter__(self):
        ...
        operation_id = getattr(self.request.state, 'operation_id', 'N/A') if self.request else 'N/A'
        user_id = getattr(self.request.state, 'user_id', 'anonymous') if self.request else 'system'
